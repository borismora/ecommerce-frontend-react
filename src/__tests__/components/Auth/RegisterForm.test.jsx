...
describe('RegisterForm', () => {
  ...
  it('calls api.post and shows success on valid submit', async () => {
    ...
    render(<RegisterForm />);
    fireEvent.change(screen.getByPlaceholderText('register.name'), {
      target: { value: fakeUser.name },
    });
    fireEvent.change(screen.getByPlaceholderText('register.email'), {
      target: { value: fakeUser.email },
    });
    fireEvent.change(screen.getByPlaceholderText('register.password'), {
      target: { value: fakeUser.password },
    });
    fireEvent.click(screen.getByText('register.submit'));

    await waitFor(() => {
      expect(api.post).toHaveBeenCalledWith('/auth/register', fakeUser);
      expect(localStorage.getItem('token')).toBe('fake-token');
      expect(screen.getByText('register.success')).toBeInTheDocument();
    });
  });

  it('shows error message if api.post throws', async () => {
